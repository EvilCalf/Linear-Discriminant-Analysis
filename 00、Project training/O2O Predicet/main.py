...
dfoff = pd.read_csv(r"D:\MyProject\Machine Learning\data\ccf_offline_stage1_train.csv")
dftest = pd.read_csv(
    r"D:\MyProject\Machine Learning\data\ccf_offline_stage1_test_revised.csv"
)
dfon = pd.read_csv(r"D:\MyProject\Machine Learning\data\ccf_online_stage1_train.csv")
print("data read end.")
...
couponbydate = (
    dfoff[dfoff["Date_received"].notnull()][["Date_received", "Date"]]
    .groupby(["Date_received"], as_index=False)
    .count()
)
couponbydate.columns = ["Date_received", "count"]
buybydate = (
    dfoff[(dfoff["Date"].notnull()) & (dfoff["Date_received"].notnull())][
        ["Date_received", "Date"]
    ]
    .groupby(["Date_received"], as_index=False)
    .count()
...
def label(row):
    ...
    if pd.notnull(row["Date"]):
        td = pd.to_datetime(row["Date"], format="%Y%m%d") - pd.to_datetime(
