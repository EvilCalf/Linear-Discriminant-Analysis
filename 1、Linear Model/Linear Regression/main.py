...
def liner_Regression(data_x, data_y, learningRate, Loopnum):
    ...
    for num in range(Loopnum):
        PredictiveValue = np.dot(data_x, Weight.T) + b
        # 求出当前权值下得出的预测值y
        loss = (
            np.dot((data_y - PredictiveValue).T, data_y - PredictiveValue)
            / data_y.shape[0]
        )
        # loss就是实际值与预测值的差的平均数
        """
        ...
        w_gradient = -(2 / data_x.shape[0]) * np.dot(
            (data_y - PredictiveValue).T, data_x
        )
        b_gradient = (
            -2
            * np.dot((data_y - PredictiveValue).T, np.ones(shape=[data_x.shape[0], 1]))
            / data_x.shape[0]
        )
        # 得到新的权值
